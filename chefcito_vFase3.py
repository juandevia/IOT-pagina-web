...
def ROI_platodecomida(frame):
    ...
    img1 = cv2.pyrMeanShiftFiltering(img1, 20, 30, 2)
    img1 = cv2.cvtColor(img1, cv2.COLOR_HSV2BGR)
    
    # Se elimina la linea circular del plato, que encierra a los alimentos
    gimg = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)        
    _, mask = cv2.threshold(gimg, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)        
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3)), iterations=3)
    mask = cv2.bitwise_not(mask)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5)), iterations=3)
    
    # Finalmente, se identifica la ROI que contiene los alimentos
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    mask = np.zeros((img.shape[0], img.shape[1], 1), dtype=np.uint8)
    cv2.drawContours(mask, contours, -1, 255, thickness=cv2.FILLED)
    aux = cv2.bitwise_and(img, img, mask=mask)
    nmask = cv2.bitwise_not(mask)
    nmask = cv2.merge((nmask,nmask,nmask))
    aux = cv2.add(aux, nmask)
...
while (1):
    ...
    if flag == 0:
        ...
        img = fmask.copy()
        #cv2.imshow("alimentos dentro del plato", fmask)
